...
function excerpt(ExcerptOverflowMonitor) {
  return {
    ...
    link: function (scope, elem, attrs, ctrl) {
      var monitor = new ExcerptOverflowMonitor({
        getState: function () {
          return {
      ...
      elem[0].addEventListener('load', monitor.scheduleOverflowCheck, false /* capture */);
      window.addEventListener('resize', monitor.scheduleOverflowCheck);
      scope.$on('$destroy', function () {
        window.removeEventListener('resize', monitor.scheduleOverflowCheck);
      });
      ctrl.contentStyle = monitor.contentStyle;

      // Watch properties which may affect whether the excerpt
      // needs to be collapsed and recompute the overflow state
      scope.$watch('vm.contentData', monitor.scheduleOverflowCheck);
      scope.$watch('vm.enabled', monitor.scheduleOverflowCheck);
